...
import CommentsSection from "./NestedComments/NestedComments";

// ─── Challenge ID → Component Map ───────────────────────────────────────────
// To register a new challenge:
//   1. Import the component above
//   2. Add an entry here:  [challengeId]: YourComponent
const challengeComponentMap = {
  1: GroupChatChallenge,
  2: CommentsSection,
};
...
export default function PracticeWorkspace() {
  ...
  const filteredCategories = categories
    .map((cat) => {
      const filtered = cat.challenges.filter(
        (c) =>
      ...
      <main className="practice-workspace">
        <header className="workspace-header">
          <div className="workspace-title-area">
            <h1>
              {activeChallenge ? activeChallenge.name : "Challenge Workspace"}
            </h1>
            <div className="workspace-meta">
              ...
              <span className="meta-tag">ID: #{activeChallengeId}</span>
              <span
                className="meta-tag"
                style={{
        ...
        <div className="workspace-content">
          {(() => {
            const ChallengeComponent = challengeComponentMap[activeChallengeId];
            if (ChallengeComponent) return <ChallengeComponent />;
            return (
            <div className="placeholder-card">
              <span className="placeholder-icon">🛠️</span>
              <h2>Challenge #{activeChallengeId}</h2>
              <p>
                This is an empty page workspace set up for{" "}
              ...
              >
                ...
                <div>
                  1. Create a component file under{" "}
                  <span style={{ opacity: 0.8 }}>src/components/</span>
                </div>
                <div>
                  2. Import it inside{" "}
                  <span style={{ opacity: 0.8 }}>
                    src/components/PracticeWorkspace.jsx
                  </span>
                </div>
                <div>
                  3. Update the conditional switch in{" "}
                  <span style={{ opacity: 0.8 }}>WorkspaceContent</span> to
                  render your new component.
                </div>
              </div>
              <button
                ...
                onClick={() =>
                  alert(
                    "Ready for you to code! Open the editor and start writing React.",
                  )
                }
              >
